/**
 * plot pol-contr data by percentage to each committee in 2019
 *
 *
 * @author Julia
 *
...
const makeCommNum2019 = (data) => {
  console.log('This is the start of the makeCommNum2019 function');
  /*
      Container Setup:
    */
  ...
  const container = select('#pol-contr-committee-num-2019').attr(
    'class',
    'pol-contr',
  );
  ...
  container
    .append('h1')
    .text('Percentage of Political Contributions by Committee in 2019');
  ...
  /*
    Create Scales:
  */
  const year2019 = data.filter((d) => parseInt(d.report_year) === 2019);
  // console.log('year2019', year2019);

  const totalContribs = year2019.length;
  // console.log('totalContribs', totalContribs); // 3742

  const committees = year2019.map((d) => d.committee_id); // can't just be data.map because then includes committees that aren't in this year
...
export default makeCommNum2019;
// conclusions:
