/**
 * plot pol-contr data by percentage to each committee in 2015
 *
 *
 * @author Julia
 *
...
const makeCommNum2015 = (data) => {
  console.log('This is the start of the makeCommNum2015 function');
  /*
      Container Setup:
    */
  ...
  const container = select('#pol-contr-committee-num-2015').attr(
    'class',
    'pol-contr',
  );
  ...
  container
    .append('h1')
    .text('Percentage of Political Contributions by Committee in 2015');
  ...
  /*
    Create Scales:
  */
  const year2015 = data.filter((d) => parseInt(d.report_year) === 2015);
  // console.log('year2015', year2015);

  const totalContribs = year2015.length;
  // console.log('totalContribs', totalContribs); // 3742

  const committees = year2015.map((d) => d.committee_id);
  ...
  const nestedComm = nest()
    ...
    .entries(year2015);
  // console.log('nestedComm', nestedComm); // 17 committees
  ...
  // check for It Starts Today & ActBlue
  ...
  //     console.log('actBlue', actBlue); // 815
  //   }
  //   // else {
  ...
  // console.log('maxCommSize', maxCommSize);
...
export default makeCommNum2015;
